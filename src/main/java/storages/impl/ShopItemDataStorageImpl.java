...
public class ShopItemDataStorageImpl extends AbstractDataStorageImpl<ShopItem>
    ...
    public ShopItemDataStorageImpl(DataStorageConfiguration configuration, ShopDataStorage shopDataStorage,
    ...
    private Product getProduct(Long id) {
        return productDataStorage.getEntities().stream().filter(product -> product.getId().equals(id))
                .findFirst().orElse(null);
    }

    private Shop getShop(Long id) {
        return shopDataStorage.getEntities().stream().filter(shop -> shop.getId().equals(id))
                .findFirst().orElse(null);
    }
}
